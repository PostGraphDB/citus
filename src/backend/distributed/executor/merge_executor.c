...
NonPushableMergeCommandExecScan(CustomScanState *node)
{
	...
	{
		switch (distributedPlan->mergeExecMethod)
		{
			case MERGE_WORKER_REPARTITION:
			{
				ExecuteSourceAtWorkerAndRepartition(scanState);
				break;
			}

			case MERGE_COORDINATOR_REDISTRIBUTION:
			{
			...
			default:
			{
				ereport(ERROR, (errmsg("Unexpected MERGE execution method(%d)",
									   distributedPlan->insertSelectMethod)));
			}
		}
...
ExecuteSourceAtWorkerAndRepartition(CitusScanState *scanState)
{
	...
	PlannedStmt *sourcePlan = copyObject(distributedPlan->selectPlanForInsertSelect);
	 ...
	 */
	List *taskList =
		RedistributedInsertSelectTaskList(mergeQuery,
										  targetRelation,
...
ExecuteSourceAtCoordAndRedistribution(CitusScanState *scanState)
{
	...
	List *taskList =
		TwoPhaseInsertSelectTaskList(targetRelationId, mergeQuery,
									 intermediateResultIdPrefix);
								 ...
								 CitusTableCacheEntry *targetRelation)
{
	...
	if (sourceJoinColumn)
	{
		 ...
		 */
		partitionColumnIndex = PartitionColumnIndex(sourceTargetList, sourceJoinColumn);
	}
